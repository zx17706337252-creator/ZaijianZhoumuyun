...
class CalculatorTool : AgentTool {
    ...
    override suspend fun execute(params: Map<String, String>): ToolResult {
        ...
        if (expr.isNullOrEmpty()) {
            return ToolResult(name, false, "", "缺少 expr 参数")
        }
        ...
        return try {
            ...
            if (unitResult != null) {
                return ToolResult(name, true, "[计算结果]\n$expr = $unitResult")
            }
            ...
            ToolResult(
                toolName = name,
                success  = false,
                content  = "表达式「$expr」无法计算：${e.message}",
                error    = e.message,
            )
        }
    }
...
class UnitConvertTool : AgentTool {
    ...
    override suspend fun execute(params: Map<String, String>): ToolResult {
        ...
        if (valueStr.isNullOrEmpty() || from.isNullOrEmpty() || to.isNullOrEmpty()) {
            return ToolResult(name, false, "", "需要 value、from、to 三个参数")
        }
        val value = valueStr.toDoubleOrNull()
            ?: return ToolResult(name, false, "「$valueStr」不是有效的数字。")
...
class CountdownTool : AgentTool {
    ...
    override suspend fun execute(params: Map<String, String>): ToolResult {
        ...
        if (toStr.isNullOrEmpty()) {
            return ToolResult(name, false, "", "需要 to 参数（格式：yyyy-MM-dd）")
        }

        return try {
            val fmt  = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.getDefault())
            val toDate   = fmt.parse(toStr)   ?: return ToolResult(name, false, "日期格式错误，请用 yyyy-MM-dd 格式。")

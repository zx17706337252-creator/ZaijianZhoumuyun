...
class PregnancyAnswerIntentDetector(
    ...
    suspend fun isAnswering(
        ...
        val userPrompt = buildString {
            appendLine("【角色之前问的问题】")
            appendLine(pendingQuestionText.trim().take(MAX_INPUT_LENGTH))
            appendLine()
            appendLine("【用户本轮回复】")
            appendLine(userReply.trim().take(MAX_INPUT_LENGTH))
            appendLine()
    ...
    suspend fun didAsk(
        ...
        val userPrompt = buildString {
            ...
            appendLine(expectedQuestionTopic.trim().take(MAX_INPUT_LENGTH))
            appendLine()
            appendLine("【角色本轮实际回复】")
            appendLine(aiReply.trim().take(MAX_INPUT_LENGTH))
            appendLine()

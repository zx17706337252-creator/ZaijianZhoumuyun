...
class EvaluationEngine(
    ...
    /**
     * 在 AI 回复完成后调用。
     * 满足触发条件时插入一条 PENDING Session，并立即触发 Agent B 评审。
     *
     * @param characterId   当前角色 ID
     * @param replyContent  AI 的完整回复文本
     * @param messageId     AI 回复的消息 ID（存入 triggerMessageId）
     * @return 新建的 EvaluationSession ID，null = 未触发
     */
    suspend fun maybeCreateSession(
        characterId: Int,
        replyContent: String,
        messageId: String,
        ...
        // 条件一：回复 ≥ 150 字
        if (replyContent.length < 150) return@withContext null
        ...
        if (claimed != now) return@withContext null

        // 条件二：有激活目标
        val activeGoals = learningGoalDao.getActive(characterId)
        if (activeGoals.isEmpty()) return@withContext null
    ...
    suspend fun runAgentReview(
        ...
        if (session.status != "PENDING") return@withContext false
    ...
    suspend fun submitUserScore(
        ...
        if (session.status != "REVIEWED") return@withContext null

 ...
 * - 第 6-13 天：安全期（SAFE）
 * - 第 14-19 天：排卵期（FERTILE，含排卵前后窗口）
 * - 第 20-28 天：安全期（SAFE）
 * 周期长度和各阶段天数允许逐人微调（见 cycleLengthDays 等参数），
 * 默认值符合常见医学参考范围。
 *
 * 问题30订正（须知第1条）：原注释写的是"第13-18天"排卵期，与当前默认
 * 参数（menstrualDays=5, fertileDays=6, cycleLengthDays=28）代入下方
 * currentPhase() 算法后的真实计算结果"第14-19天"不一致，差1天。
 * 经逐天枚举核实（经期5天 + 安全期前段8天 + 排卵期6天 + 安全期后段9天
 * = 28天，无重叠无缺漏），确认代码算法本身自洽、正确——整数除法
 * (28-5-6)/2=8 向下取整后，前段安全期比后段安全期少1天是预期内的
 * 近似（17天间隔无法被2整除时的正常取舍），不是需要修复的 bug。
 * 真正的问题是本注释里的示例数字写错，未与算法结果对齐，现已订正为
 * 与算法结果精确一致的"第14-19天"。不改动 currentPhase() 的计算逻辑，
 * 避免"修复"一个实际并不存在的计算错误、反而破坏已验证自洽的划分。
 */

 ...
 */
private fun hasPathTraversal(path: String): Boolean {
 ...
 */
private fun resolveFileSystemPath(context: Context, path: String): File? {
    ...
    override suspend fun execute(params: Map<String, String>): ToolResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            ZipOutputStream(FileOutputStream(zipFile)).use { zos ->
                val files = if (source.isDirectory) source.listFiles()?.toList() ?: emptyList() else listOf(source)
                for (f in files) {
                    if (f.isFile) {
        ...
        try {
            ...
            for ((index, item) in sorted.withIndex()) {
                ...
                if (newName != item.name) {
                    item.renameTo(File(item.parentFile, newName))
                    renamed++
                }
            }

...
class ProviderManager private constructor(context: Context) {
    ...
    /**
     * P1-13-18 修复：Key 或活跃 Provider 变更时的回调。
     * 由 ZaijianApp 注入，用于在用户首次配置 Key 后触发竞争引擎延迟装配。
     * 在 IO 线程（apply() 回调）被调用，注入方需自行切换线程。
     */
    @Volatile var onProviderConfigChanged: (() -> Unit)? = null
    ...
    private val prefs: SharedPreferences
        get() {
            return prefsCache ?: synchronized(this) {
                prefsCache ?: EncryptedSharedPreferences.create(
                    appContext,
                    "zaijian_api_keys",
    ...
    fun saveKey(providerId: String, apiKey: String) {
        ...
        onProviderConfigChanged?.invoke()  // P1-13-18
    }

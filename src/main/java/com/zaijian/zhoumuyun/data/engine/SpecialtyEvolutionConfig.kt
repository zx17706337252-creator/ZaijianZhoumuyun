...
object SpecialtyEvolutionConfig {
    ...
    /**
     * 风格分歧（CONFLICTING 记录）至少要保留到"下两次阶段摘要周期之后"才允许被蒸馏降级，
     * 这里用"经历过几次 RAW_TO_DIGEST 触发"来计数，而非绝对天数
     * （活跃度不同的专长，"两次周期"对应的实际天数本就不同，这正是容量驱动设计的意图）。
     */
    const val CONFLICT_RETENTION_DIGEST_CYCLES = 2
    ...
    const val JUDGMENT_TEMPERATURE = 0.4

    /** 每日修炼"创作产出"本身使用的 temperature，比判断类调用更高，鼓励创作多样性 */
    const val PRACTICE_TEMPERATURE = 0.9

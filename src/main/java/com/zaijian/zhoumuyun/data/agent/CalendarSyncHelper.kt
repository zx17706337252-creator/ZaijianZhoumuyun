...
class CalendarSyncHelper(private val context: Context) {
     ...
     * 支持：整小时（FREQ=HOURLY）、整天（FREQ=DAILY）、整周（FREQ=WEEKLY）、整月（FREQ=MONTHLY）。
     * 其余间隔返回空字符串（不设 RRULE，日历侧显示为一次性事件）。
     */
    private fun buildRRule(intervalMs: Long): String {
        val hours = intervalMs / (60 * 60 * 1000L)
        return when {
            hours <= 0L     -> ""
            hours % 720 == 0L -> "FREQ=MONTHLY;INTERVAL=${hours / 720}"   // 30天倍数
            hours % 168 == 0L -> "FREQ=WEEKLY;INTERVAL=${hours / 168}"    // 7天倍数
            hours % 24  == 0L -> "FREQ=DAILY;INTERVAL=${hours / 24}"      // 整天
            else              -> "FREQ=HOURLY;INTERVAL=${hours}"           // 整小时
        }
    }
}

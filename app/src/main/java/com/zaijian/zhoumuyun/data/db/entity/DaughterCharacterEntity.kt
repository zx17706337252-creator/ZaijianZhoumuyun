...
data class DaughterCharacterEntity(
    ...
    /**
     * 女儿/孙女代际称呼（P1-47 修复新增，v48→v49）。
     *
     * W6-02 复核修正：本系统不存在"性别可选"设计，全部固定为女儿系统，
     * 该字段实际存的是代际称呼词本身（"女儿"/"孙女"），不是"男"/"女"这种
     * 性别符号——FamilyScreen.kt 直接用 `member.gender ?: "孩子"` 当展示
     * 文案使用，不做二次映射。
     * 由 DaughterCharacterGenerator.parseAndValidate() 根据 motherCharacterId
     * 是否 >= 1000 判断代际后写入："女儿"（第二代）/ "孙女"（第三代）。
     * null 表示旧数据（此字段上线前生成，FamilyScreen 兜底显示"孩子"）。
     * 
     * P3-11 修复：字段重命名 gender → kinshipTerm，明确语义为代际称呼
     * （"女儿"/"孙女"）而非性别（male/female）。
     * 数据库列名保持 "gender" 不变（通过 @ColumnInfo 映射），避免 Migration。
     */

...
class PregnancyTriggerManager(
    ...
    // ── D2.5/D2.6 共用：亲密/家庭/靠近语义触发词 ───────────────
    // P2-extra 修复：提取为类级常量，消除 buildCrossPhaseBackgroundPrompt()
    // 和 buildMiscarriageCrossPhasePrompt() 中重复定义的两份列表。
    ...
    suspend fun checkTrigger(characterId: Int, text: String): TriggerInfo {
        ...
        // 这类否定表达会误判为命中"想要孩子"，导致触发方向完全反转。
        // 改为否定感知匹配：命中词前方出现"不/没/别"等否定词时不计入触发。
        val hit = keywords.any { NegationUtils.containsUnnegated(text, it) }
     ...
     *
     * W6-05 修复：`markFertileWindowConsentAsked` 落库此前完全依赖调用方
     * （ChatViewModel）在拿到结果后自觉调用，只是文档约定、无代码保证——
     * 如果未来新增调用方遗漏这一步，会导致同一排卵期窗口重复弹窗。
     * 现在改为本方法内部无条件执行（用 finally 保证即使 evaluateCycleAndProceed/
     * applyRejectedEffect 抛异常也不会被跳过），把契约从注释升级为代码保证。
     *
     * S8-窗口09 新发现3修复：此前本注释声称"ChatViewModel 里原有的
     * markFertileWindowConsentAsked 调用予以保留"，但全项目 grep 确认该调用
     * 在 ChatViewModel 中从未存在过（窗口09审查结论1/结论6同时发现，
     * ChatViewModel.onFertileWindowDialogResult() 此前从未调用本方法，
     * 遑论调用 markFertileWindowConsentAsked）。该表述已随本次修复一并
     * 移除，避免继续误导后续维护者。落库保证完全由本方法 finally 块提供，
     * 调用方（现为 ChatViewModel.onFertileWindowDialogResult()）无需
     * 也不应重复调用 markFertileWindowConsentAsked。
     */
    ...
    private suspend fun evaluateCycleAndProceed(
        ...
        return if (Random.nextFloat() < successRate) {
            // ── 成功 ─────────────────────────────────────────
            ...
            // ── 失败 ─────────────────────────────────────────
            // P0-2 修复：失败分支原先 updateState + updateFailCount 两步无事务包裹，
            // 若 updateState 成功但 updateFailCount 被打断，desire/suppression 已变
            // 但 failCount 未更新，下次失败时梯度计算（when newFailCount）会对应错误
            // 的梯度级别，丢失一次失败计数。
            // W6-06 修复：newFailCount 此前在事务内算出后未随事务返回值带出，
            // 导致事务结束后又调用 pregnancyRepository.getPregnancy() 重新查一次库，
            // 只为拿回同一个刚刚在事务里算过的值。现在改为 Triple 把 newFailCount
            // 一并带出，事务外不再重复读库。
            val (newDesire, newSuppres, newFailCount) = db.withTransaction {
                val pregnancyState = pregnancyRepository.getPregnancy(characterId)
                ...
                val nd = min(100, desire  + extraDesire)
                val ns = min(100, suppres + extraSuppres)

                stateRepository.updateState(characterId, state.copy(
                    motivationalState = state.motivationalState.copy(desireStrength = nd),
                    hiddenState       = state.hiddenState.copy(emotionalSuppression = ns),
                ))
                pregnancyRepository.updateFailCount(characterId, newFailCount)
                Triple(nd, ns, newFailCount)
            }
    ...
    fun shouldInjectFailureContext(
        ...
        if (INTIMATE_KEYWORDS.none { userText.contains(it) }) return null

        // 门控③：随机概率（乘以 pressureScale）
        val effectiveProbability = (INJECT_BASE_PROB * pressureScale).coerceIn(0f, 1f)
        if (Random.nextFloat() >= effectiveProbability) return null
    ...
    fun shouldInjectMiscarriageContext(
        ...
        // P2-7 修复：miscarriageDaysAgo 改为 Int?，null 表示未流产，
        // 未流产时不注入悲伤余波上下文。
        if (daysAgo == null || daysAgo > MISCARRIAGE_AFTERMATH_WINDOW_DAYS) return null
    ...
    companion object {
        /**
         * L-P1-6 修复：独立于聊天触发的 fertileWindowConsentAsked 清除入口。
         *
         * 背景：原清除逻辑寄生在 [shouldEvaluateFertileWindowConsent] 内部，只在该方法
         * 被调用（即用户与 characterId>=1000 的角色聊天）时才可能触发。若用户在两次
         * 排卵期之间从未在"非排卵期"打开过与该角色的聊天，标记会永久停留在 true，
         * 导致下一次排卵期窗口无法再次弹出同意弹窗。
         *
         * 本方法不依赖任何聊天路径，由 ProactiveMessageWorker 的后台周期心跳调用
         * （目前每 90 分钟一次，与主动消息检查同一周期），独立遍历全部 characterId>=1000
         * 的角色，检查其当前是否处于排卵期窗口；不在窗口内且标记为 true 时，清除标记。
         *
         * 仅处理 characterId >= 1000（第二代/第三代女儿），1-6 号角色不使用这套
         * fertileWindowConsentAsked 机制（走关键词兜底链路），无需清理。
         *
         * 每个角色的清理失败（DB 异常等）互相隔离，不影响其他角色的处理。
         */
        suspend fun resyncFertileWindowFlags(db: AppDatabase) {
            try {
                val pregnancyRepository = PregnancyRepository(db.pregnancyDao())
                val cycleRepository = MenstrualCycleRepository(db.menstrualCycleDao())
                val daughterIds = db.daughterCharacterDao().getAllDaughterCharacterIds()

                for (characterId in daughterIds) {
                    if (characterId < 1000) continue // 双重保险：daughter 表本身应已只含 >=1000 的 ID
                    try {
                        val pregnancyState = pregnancyRepository.getPregnancy(characterId)
                        if (pregnancyState.isPregnant) continue // 怀孕期间不参与排卵期窗口判定

                        if (!pregnancyState.fertileWindowConsentAsked) continue // 未标记，无需清理

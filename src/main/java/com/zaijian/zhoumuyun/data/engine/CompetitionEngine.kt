        ...
        try {
            val response = provider.chatSyncWithRetry(
                ...
                config = LLMConfig(
                    ...
                    temperature = SpecialtyEvolutionConfig.JUDGMENT_TEMPERATURE.toFloat(),
                    stream = false,
                ),
            )
            ...
            for (i in 0 until verdictsArray.length()) {
                ...
                verdicts.add(
                    EntryVerdict(
                        ...
                        rank = v.optInt("rank", i + 1).coerceIn(1, n),
                    )
                )
            }

            // 若模型漏掉了某些参赛者，补上默认条目（避免后续空指针）。
            // 注意：这一步会让 verdicts.size 补齐到等于 entries.size，
            // 所以 success 判定必须基于补全之前的真实命中数 validVerdictCount，
            // 否则"LLM 完全没解析出有效结果、全靠默认值兜底"的情况也会被误判为成功。
            val validVerdictCount = verdicts.size
            val returnedIds = verdicts.map { it.characterId }.toSet()
            ...
            // M5 修复：success 要求至少覆盖半数以上参赛者的真实评审结果，
            // 而不是"补全后非空"这种必然为真的弱判定。低于半数视为本轮评审
            // 质量不可信，交还调用方（CompetitionRoundManager）走重试逻辑。
            val success = validVerdictCount > 0 && validVerdictCount * 2 >= n
            ...
            // 解析失败时返回空结果，success=false，调用方（CompetitionRoundManager）应重试

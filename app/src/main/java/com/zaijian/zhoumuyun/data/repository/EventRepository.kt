 ...
 */
// P3-32 修复：命名澄清——EventRepository 管理的是 WorldEvent（世界事件/日程条目），
// 而非 ScheduleRepository 管理的 ScheduledJob（定时任务/后台 Job）。
// 两者都在 data/repository/ 下，但职责完全不同：
//   - EventRepository → WorldEventDao（日程事件 CRUD）
//   - ScheduleRepository → ScheduledJobDao（后台 Job 调度 CRUD）

...
import com.zaijian.zhoumuyun.ui.screen.characterdetail.RELATION_TAB_INDEX
...
// 批次4-8-14 修复：路由参数缺失时不再静默白屏，改为渲染错误占位组件。
// 原先 10 处路由参数缺失时使用 return@composable 直接退出，用户看到的是
// 完全空白的页面，没有任何反馈。现在统一渲染错误提示，用户知道出了什么问题。
@Composable
private fun RouteParamError(paramName: String) {
    ...
    Box(
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Text("页面加载失败", style = type.cardTitle, color = colors.textPrimary)
            Text("缺少必要参数：$paramName", style = type.caption, color = colors.textSecondary)
        }
    }
}
...
/**
 * 底部 Tab 切换专用：navigateSingle 的姊妹函数，语义一致（try-catch 防御 +
 * 静默日志降级），但保留 popUpTo(startDestination){ saveState = true } +
 * restoreState 这套 Tab 切换需要的完整 NavOptions——navigateSingle 的单参数
 * 签名只支持 launchSingleTop，表达不了这些选项，这也是此前底部导航栏改用
 * 原生带 lambda 的 navigate() 而非 navigateSingle 的原因。
 *
 * window14新问题W14-F4修复：底部导航栏原直接调用 navController.navigate()，
 * 无 try-catch，与 navigateSingle 的防御风格不一致。虽然 item.targetRoute
 * 均为 bottomNavItems 中的硬编码常量、已在 NavHost 注册，实际不会触发
 * IllegalArgumentException，但收口为统一扩展函数，避免未来 targetRoute
 * 来源变化（例如改为动态拼接）时失去这层防护。
 */
fun NavController.navigateBottomTab(route: String) {
    try {
        ...
        android.util.Log.e("AppNavigation", "navigateBottomTab 路由无效，已忽略: route=$route", e)
    }
}
...
sealed class AppRoute(val route: String) {
    ...
    object CharacterDetail : AppRoute("character_detail/{characterId}?tab={tab}") {
        fun createRoute(id: Int, tab: Int? = null): String {
            val base = "character_detail/$id"
            return if (tab != null) "$base?tab=$tab" else base
        }
    }
...
// P3-29 修复：所有路由前缀以 "/" 结尾，确保 startsWith 精确匹配，
// 如 "projects/123" 不会误匹配 "project_detail/456"
// 而 "project_detail" 开头不同。
// 注意：若未来新增无 "/" 结尾的路由，
...
private val detailRoutes = listOf(
    ...
    // P3-29 修复：projects/timeline/global_schedule 不带 "/" 结尾
    // 因为 AppRoute 中这三个路由模板本身无斜杠（如 AppRoute.GlobalSchedule("global_schedule")），
    // 加 "/" 会导致 startsWith 匹配失败，转场动画退化。
)
...
private val bottomNavItems = listOf(
    ...
    BottomNavItem(AppRoute.Tasks.route,         AppIcons.CheckCircle, "事务"),
...
fun AppNavigation(
    ...
    LaunchedEffect(currentPendingRoute) {
        if (currentPendingRoute != null) {
            ...
            try {
                ...
                // 批次4-6 修复：onPendingRouteConsumed() 原先在 try-catch 之后调用，
                // 导航失败（IllegalArgumentException）时也会消费 pendingRoute，
                // 导致下次重组时 pendingRoute 已为 null，永久丢失重试机会。
                // 改为仅在导航成功时消费，失败时保留 pendingRoute 供下次重试。
            }
        }
    }
    ...
    // sharedAppearanceDataStore 单例，不再各自 remember 独立实例化，
    // 避免与 ProfileScreen 各持一份实例导致的配置变更短暂不一致。
    val appearanceStore = com.zaijian.zhoumuyun.ZaijianApp.sharedAppearanceDataStore
        ?: remember { com.zaijian.zhoumuyun.data.datastore.AppearanceDataStore(context.applicationContext) }
    ...
    Scaffold(
        ...
        bottomBar = {
            if (showBottomBar) {
                ...
                // Fix-底栏穿透：原 alpha=0.88f 是玻璃拟态遗留值，用户明确要求
                // 底栏必须完全不透明遮挡，不再做半透明处理。
                ...
                Box(
                    modifier = Modifier
                        ...
                        // Fix-底栏点击穿透：整条底栏（含手势条安全区那一截）此前
                        // 只是纯展示 Box，不消费触摸事件——事件会继续向下命中
                        // NavHost 里位于底栏视觉区域"背后"的可点击内容（如
                        // WorldScreen/CharacterScreen 里贴底部的卡片）。加一个
                        // 空 clickable 拦截，让底栏范围内的点击到此为止，不再下穿。
                        // indication=null 避免出现不需要的水波纹反馈。
                        .clickable(
                        ...
                        Column(
                            ...
                            modifier = Modifier
                                ...
                                .clickable(
                                    ...
                                    navController.navigateBottomTab(item.targetRoute)
                                }
        ...
        NavHost(
            ...
            // ── Notifications（通知中心，设计方案全文）──
            composable(AppRoute.Notifications.route) {
                NotificationScreen(
                    onBack = { navController.popBackStack() },
                    onNavigateToCharacterRelationTab = { charId ->
                        navController.navigateSingle(AppRoute.CharacterDetail.createRoute(id = charId, tab = RELATION_TAB_INDEX))
                    },
            ...
            composable(AppRoute.World.route) {
                WorldScreen(
                    ...
                    onNavigateToNotifications = { navController.navigateSingle(AppRoute.Notifications.route) },
            ...
            composable(
                ...
                val id = backStackEntry.arguments?.getString("projectId") ?: run {
                RouteParamError("projectId")
                return@composable
            }
                ProjectDetailScreen(
                    projectId = id,
            ...
            composable(
                ...
                val id = backStackEntry.arguments?.getInt("characterId") ?: run {
                RouteParamError("characterId")
                return@composable
            }
                ChatScreen(
                    characterId         = id,
                    ...
                    // 批次4：跳转到该角色的个人日程页。
                    // 照抄上方 onNavigateToProfile 的写法（navigateSingle + createRoute），
                    // AppRoute.PersonalSchedule 路由已在第218行注册、第924/931行
                    // 接入 PersonalScheduleScreen，本批次只是补上 ChatScreen 的入口回调。
            ...
            composable(
                ...
                arguments = listOf(
                    navArgument("characterId") { this.type = NavType.IntType },
                    navArgument("tab") {
                        this.type = NavType.IntType
                        this.defaultValue = -1   // Navigation 组件的 NavType.IntType 不支持
                                                  // nullable，用 -1 表示"未传"，读取时转回 null
                    },
                ),
            ) { backStackEntry ->
                val id = backStackEntry.arguments?.getInt("characterId") ?: run {
                ...
                val rawTab = backStackEntry.arguments?.getInt("tab") ?: -1
                val tab = rawTab.takeIf { it >= 0 }
            ...
            composable(
                ...
                val raw = backStackEntry.arguments?.getString("memberIds") ?: run {
                RouteParamError("memberIds")
                return@composable
            }
                val ids = raw.split(",").mapNotNull { it.trim().toIntOrNull() }
                if (ids.isEmpty()) {
                RouteParamError("memberIds (解析结果为空)")
                return@composable
            }
                RoundtableScreen(
                    characterIds = ids,
            ...
            composable(
                ...
                val motherId = backStackEntry.arguments?.getInt("motherId") ?: run {
                RouteParamError("motherId")
                return@composable
            }
                FamilyScreen(
                    motherId           = motherId,
            ...
            composable(
                ...
                val charId = backStackEntry.arguments?.getInt("characterId") ?: run {
                RouteParamError("characterId")
                return@composable
            }
                FileVaultScreen(
            ...
            composable(
                ...
                val characterId = backStackEntry.arguments?.getInt("characterId") ?: run {
                RouteParamError("characterId")
                return@composable
            }
            ...
            composable(
                ...
                JudgeProfileScreen(
                    characterId = charId,
                    onBack      = { navController.popBackStack() },
                    onNavigateToCompetition = { roundDomain ->
                        // D-1 P0 fix：避免与 Competition 形成循环跳转（无限入栈）。
                        // JudgeProfile 从 Competition 进入，返回时 popBackStack 即可回到来源 Competition 页，
                        // 不再 navigateSingle 新建入栈。
                        // P2-7-8 修复：原实现忽略点击轮次的真实 domain，无条件 popBackStack——裁判列表含多
                        // domain 档案时，从 Competition(X) 进、点 domain=Y 的轮次，本应看 Y 却 pop 回 X。
                        // 现校验来源 Competition 的 domain：相等则回到来源页；不等则 navigateSingle 到
                        // Competition(Y)（launchSingleTop 防同 domain 重复入栈）。
                        val sourceDomain = navController.previousBackStackEntry
                            ?.arguments?.getString("domain")
                        if (sourceDomain != null && sourceDomain == roundDomain) {
                            navController.popBackStack()
                        } else {
                            navController.navigateSingle(AppRoute.Competition.createRoute(roundDomain))
                        }
                    },
                )
            }

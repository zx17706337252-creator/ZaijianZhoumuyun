 ...
 *
 * 原 Phase22Tools.kt + Phase25Tools.kt 合并重命名。
 * 包含 5 个工具：
    ...
    override suspend fun execute(params: Map<String, String>): ToolResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            filtered.forEach { memory ->
                try { memoryDao.recordAccess(memory.id, now) } catch (_: Exception) {}
            }
...
class GoalUpdateTool(
    ...
    override suspend fun execute(params: Map<String, String>): ToolResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // 读取更新后的目标（用于显示最新进度）
            val updated = goalDao.getById(goalId)

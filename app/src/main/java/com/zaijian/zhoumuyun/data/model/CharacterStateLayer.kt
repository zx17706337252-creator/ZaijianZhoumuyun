...
// ─────────────────────────────────────────────────────────────
// P3-11 修复：所有角色的默认初始值完全相同，运行时由引擎根据角色定义
// 覆盖（如蒂法 talkativeness 应 > 50，伊芙 vigilance 应 > 30）。
// 当前依赖 DB 持久化状态覆盖默认值，但首次创建角色时（DB 无记录）所有角色
// 初始状态完全一致，不符合角色差异化设计。TODO: 为每个角色提供独立默认值。

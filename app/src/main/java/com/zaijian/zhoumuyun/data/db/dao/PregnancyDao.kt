...
abstract class PregnancyDao {
    ...
    /**
     * P1-4 修复：原子触发流产。
     * 将"读取当前状态→覆盖写流产字段"合并为单个事务，
     * 消除 getPregnancy → upsertPregnancy 间的 TOCTOU 竞态。
     */
    @Transaction
    open suspend fun triggerMiscarriageAtomic(characterId: Int, now: Long) {
        val current = getPregnancy(characterId)
            ?: PregnancyEntity(
    ...
    // 方案 5-9：删除 updateMiscarriedAt 死代码。
    // 全项目零调用点，已被 triggerMiscarriageAtomic() 在同一 @Transaction 内替代。

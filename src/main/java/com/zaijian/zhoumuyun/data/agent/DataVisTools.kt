 ...
 *   在 ZaijianApp.onCreate() 中替换原 registerPhase28Part2Tools(...) 调用。
 */
...
// 性能 L3 修复：原 saveRawBytes(bytes: ByteArray) 已被下面的 saveViaStream 取代
// （ExcelGenTool/PptxGenTool 均已切换为直接写文件流），不再有调用方，整体移除避免死代码。
...
fun AgentToolRegistry.registerDataVisTools(
    ...
    val providerFn: () -> com.zaijian.zhoumuyun.data.provider.LLMProvider? =
        { com.zaijian.zhoumuyun.data.provider.ProviderManager.instance.activeProvider }
    registerAll(

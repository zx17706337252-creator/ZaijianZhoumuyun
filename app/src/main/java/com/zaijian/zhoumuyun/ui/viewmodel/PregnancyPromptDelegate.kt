...
class PregnancyPromptDelegate(
    ...
    suspend fun buildPregnancyPrompts(
        ...
        // 不是持续轮询状态。
        val (pregnancyStateAfterConsent, pregnancyTriggerPromptPatch) = resolveKeywordTriggerConsent(
            characterId              = characterId,
            userText                 = userText,
            pregnancyState           = pregnancyState,
            pendingKeywordTriggerMap = pendingKeywordTriggerMap,
        )
        pregnancyState = pregnancyStateAfterConsent
        ...
        val (miscarriageAftermathPatch, failureContextPatch) = buildRecurringContextPatches(
            characterId   = characterId,
            userText      = userText,
            pregnancyState = pregnancyState,
        )
        ...
        val routinePressurePatch = buildRoutinePressurePatch(characterState)
        ...
        // ══════════════════════════════════════════════════════════════
        val isD3Eligible = computeD3Eligibility(characterId, pregnancyState)
        ...
        if (pendingQuestionAtTurnStart != null) {
            processD3PendingAnswer(
                characterId          = characterId,
                userText             = userText,
                pregnancyState       = pregnancyState,
                pendingQuestion      = pendingQuestionAtTurnStart,
                onTriggerD4Generation = onTriggerD4Generation,
            )
        }

        // 步骤②：仅当本轮开始时没有挂起问题，才考虑问下一个槎位的新题
        val (d3QuestionPatch, d3PendingAsk) = askNextD3Question(
            characterId       = characterId,
            isD3Eligible      = isD3Eligible,
            hasPendingQuestion = pendingQuestionAtTurnStart != null,
        )
    ...
    /**
     * 常规压力 Prompt（P1-5 拆分：纯函数，无门控、每轮都渲染）。
     *
     * 基于 characterState 的 desireStrength/emotionalSuppression 数值分档给出背景文案。
     * 与 failureContextPatch 的区别：那是"事件驱动、有冷却"的一次性情绪涟漪，
     * 这是"持续存在、无冷却"的背景压力描述，两者不互斥、可以同轮共存。
     */
    private fun buildRoutinePressurePatch(characterState: CharacterStateLayer): String =
        if (
    ...
    /**
     * D3 孕期共设 · 三重门控判定（P1-5 拆分：纯函数）。
     *
     * ① 孕期状态不符（非母亲角色 / 未怀孕 / 第三代女儿——没有第四代可问）→ 不触发
     */
    private suspend fun computeD3Eligibility(characterId: Int, pregnancyState: PregnancyState): Boolean =
        isDaughterMother(characterId) &&
            pregnancyState.isPregnant &&
            (characterId < 1000 || !daughterRepo.isThirdGeneration(characterId))

    /**
     * D3 步骤②：问下一个槎位的新题（P1-5 拆分）。
     *
     * 仅当本轮开始时没有挂起问题（[hasPendingQuestion] == false）且 D3 资格满足时，
     * 才考虑问下一个槎位的新题。返回 (d3QuestionPatch, d3PendingAsk)。
     */
    private suspend fun askNextD3Question(
        characterId: Int,
        isD3Eligible: Boolean,
        hasPendingQuestion: Boolean,
    ): Pair<String, Pair<PregnancyQuestionType, Int>?> {
        var d3QuestionPatch = ""
        // (questionType, slotIndex) —— 本轮若确实问出口，AI 回复生成后用于落库
        var d3PendingAsk: Pair<PregnancyQuestionType, Int>? = null
        if (isD3Eligible && !hasPendingQuestion) {
        ...
        return d3QuestionPatch to d3PendingAsk
    }

    /**
     * D3 步骤①：处理上一轮挂起的槎位回答（P1-5 拆分）。
     *
     * 若上一轮有挂起问题，本轮用户消息可能是在回答——交给 AI 判定意图。
     * 判定为 YES 且确认真实回答时：落库答案、清除挂起状态；若本次锁定后
     * 全部 6 个槎位均已锁定，则读取全部锁定答案并通过 [onTriggerD4Generation]
     * 回调触发 D4 人格生成器（回调由调用方负责放进独立协程，查询/生成都是 IO 密集）。
     */
    private suspend fun processD3PendingAnswer(
        characterId: Int,
        userText: String,
        pregnancyState: PregnancyState,
        pendingQuestion: com.zaijian.zhoumuyun.data.db.entity.PregnancyPendingQuestionEntity,
        onTriggerD4Generation: suspend (Map<String, String>) -> Unit,
    ) {
        val intent = pregnancyAnswerIntentDetector.isAnswering(
            pendingQuestionText = pendingQuestion.questionText,
            userReply           = userText,
        )
        if (intent == IntentResult.YES) {
            val answeredType = runCatching {
                PregnancyQuestionType.valueOf(pendingQuestion.questionType)
            }.getOrNull()
            if (answeredType != null) {
                val answeredSlot = pendingQuestion.slotIndex
    ...
    /**
     * 1-6 号角色关键词兜底触发链路 —— evaluateConsent()（P1-5 拆分）。
     *
     * 消费上一轮 checkTrigger()（后置分析协程块内）留下的 pending 标记：
     * 若上一轮 AI 回复命中了触发词，本轮用户消息（text）就是"回应"，
     * 送入 evaluateConsent() 走完整判定链（突破检测 → 同意/拒绝/模糊）。
     ...
     * 不是持续轮询状态。
     *
     * @return (更新后的 pregnancyState, pregnancyTriggerPromptPatch)
     *   - Triggered 时 pregnancyState 由 manager 落库后重新读取（保证用最新状态）；
     *   - FertileButFailed/BreakthroughA/BreakthroughB 时 patch 非空。
     */
    private suspend fun resolveKeywordTriggerConsent(
        characterId: Int,
        userText: String,
        pregnancyState: PregnancyState,
        pendingKeywordTriggerMap: ConcurrentHashMap<Int, Boolean>,
    ): Pair<PregnancyState, String> {
        var current = pregnancyState
        var patch = ""
            ...
            try {
                ...
                when (triggerResult) {
                    is PregnancyTriggerResult.Triggered -> {
                        ...
                        current = pregnancyRepo.getPregnancy(characterId)
                    }
                    is PregnancyTriggerResult.FertileButFailed -> {
                        patch = triggerResult.immediatePromptPatch
                    }
                    is PregnancyTriggerResult.BreakthroughA -> {
                        patch = triggerResult.promptPatch
                    }
        ...
        return current to patch
    }

    /**
     * D2.6 流产余波 + D2.5 失败背景补丁（P1-5 拆分）。
     *
     * 红线：pressureScale / nowSnapshot 在方法内统一取一次（整轮快照），
     * 显式透传给 shouldInjectMiscarriageContext()/shouldInjectFailureContext()，
     * 避免独立方法各自调用 System.currentTimeMillis() 产生毫秒级不一致。
     *
     * @return (miscarriageAftermathPatch, failureContextPatch)
     */
    private suspend fun buildRecurringContextPatches(
        characterId: Int,
        userText: String,
        pregnancyState: PregnancyState,
    ): Pair<String, String> {
        ...
        return miscarriageAftermathPatch to failureContextPatch
    }
    ...
    suspend fun runPostReplyAnalysis(
        ...
        // ── 1.1 受孕窗口同意对话框触发链路 ────────────────────────────
        evaluateFertileWindowConsentDialog(
            characterId                = characterId,
            lastFertileJudgeAtMap      = lastFertileJudgeAtMap,
            recentMessages             = recentMessages,
            character                  = character,
            onFertileWindowConsentDialog = onFertileWindowConsentDialog,
        )

        // ── 检查5a：D5 关系阶段引擎 ─────────────────────────────────
        advanceRelationStageAndMaybeSpawnNextGeneration(
            characterId           = characterId,
            userText              = userText,
            aiReply               = aiReply,
            onTriggerD4Generation = onTriggerD4Generation,
     ...
     * 宽 catch（附2 修复）：对齐 checkTrigger 先例，捕获非 DaughterDataException
     * 的裸 Room DAO 异常，避免调用方协程静默终止。
     */
    private suspend fun advanceRelationStageAndMaybeSpawnNextGeneration(
        characterId: Int,
        userText: String,
        aiReply: String,
        onTriggerD4Generation: suspend (Map<String, String>) -> Unit,
        ...
        try {
            ...
            ZLog.e("PregnancyPromptDelegate", "D5 升阶检查异常（不影响主流程，D3 didAsk 判定继续），daughterId=$characterId", e)
        }
    }

    /**
     * 受孕窗口同意对话框触发链路（P1-5 拆分）。
     *
     * 三重门控顺序：门1+门2（shouldEvaluateFertileWindowConsent）→ 门3（AI语义判定）。
     * L5 修复：加冷却保护，避免同一角色每条消息都触发一次 AI 判定 LLM 调用。
     */
    private suspend fun evaluateFertileWindowConsentDialog(
        characterId: Int,
        lastFertileJudgeAtMap: ConcurrentHashMap<Int, Long>,
        recentMessages: List<LLMMessage>,
        character: CharacterConfig?,
        ...
        // 崩溃修复：此前整段无 try-catch，直接跑在 ChatMessageOrchestrator 里
        // 那个裸 viewModelScope.launch(Dispatchers.IO) 中（外层 runCatching 只包
        // 评分卡链路，不包这里）。judgeFertileWindowIntent 等内部可能触发的
        // Room/网络异常会一路抛到协程顶层，导致 CoroutineExceptionHandler
        // 缺失时的进程级崩溃。对齐同文件 scanKeywordTriggerForNextTurn /
        // advanceRelationStageAndMaybeSpawnNextGeneration 的既有保护风格。
        try {
            ...
            ZLog.w("PregnancyPromptDelegate", "evaluateFertileWindowConsentDialog 异常（不影响主流程）", e)
        }
    }

    /**
     * 1-6 号角色关键词兜底触发链路 —— checkTrigger()（P1-5 拆分）。
     *
     * AI 回复（capturedReply）写库完成后，扫描本轮回复文本是否命中
     * CharacterTriggerKeywords 关键词表。命中则把 pending 标记写入
     * pendingKeywordTriggerMap，供下一轮用户发消息时的 evaluateConsent()
     ...
     * 仅在角色未怀孕时才有意义（已怀孕不需要再判定是否触发怀孕）。
     */
    private suspend fun scanKeywordTriggerForNextTurn(
        characterId: Int,
        aiReply: String,
        pregnancyState: PregnancyState,
        pendingKeywordTriggerMap: ConcurrentHashMap<Int, Boolean>,
    ...
    /**
     * D3 didAsk 判定（P1-5 拆分：后置分析最后一段）。
     *
     * 本轮注入了提问指令，确认 AI 是否真的把问题问出口。
     * Fix（token 优化）：d3QuestionPatch 含完整指令块，截取前 200 字。
     * didAsk == NO：AI 没把问题问出口，不记 pending、不增加计数。
     */
    private suspend fun recordD3AskIfConfirmed(
        characterId: Int,
        aiReply: String,
        d3Pending: Pair<PregnancyQuestionType, Int>?,
        d3Patch: String,
    ) {
        // 崩溃修复：此前无 try-catch。pregnancyAnswerRepo.recordPendingQuestion
        // 是 Room 写操作，任一 DAO/并发写异常都会直接抛到调用方
        // ChatMessageOrchestrator 里那个裸 viewModelScope.launch(Dispatchers.IO)
        // 顶层（无 CoroutineExceptionHandler）→ 全局异常处理器 killProcess() →
        // 整 App 闪退。对齐同文件其它后置分析函数的保护风格。
        try {
            if (d3Pending != null) {
            ...
            ZLog.w("PregnancyPromptDelegate", "recordD3AskIfConfirmed 异常（不影响主流程）", e)
        }
    }

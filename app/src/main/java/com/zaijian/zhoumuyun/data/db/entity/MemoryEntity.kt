...
@Entity(
    ...
    indices = [
        ...
        Index(value = ["ftsRowId"]),
        Index(value = ["characterId", "scope"]),
        Index(value = ["characterId", "domain", "goalId", "isLocked"]),
        Index(value = ["isCore", "isEternal", "lastAccessedAt"]),
    ]
)
data class MemoryEntity(
    ...
    /** 重要度 1-5
     *
     * P2-8 RULE 语义标注：对于 domain=RULE 类型的记忆，importance 被复用为
     * **出现次数计数器**：importance = 这条规则被多个独立提炼轮次确认的次数。
     * 每出现一次 +1，上限 5（importance=5 表示出现至少 5 次）。
     * 锁定条件：importance ≥ 3（出现次数 ≥ 3）且 importance × 1.0f ≥ 锁定置信度阈值。
     */
    val importance: Int,

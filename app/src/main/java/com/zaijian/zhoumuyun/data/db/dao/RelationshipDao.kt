...
interface RelationshipDao {
    ...
    /**
     * 角标 Flow 化改造第1步：与 getAllInterCharacter 同一条 SQL 的
     * Flow 版本，供 BriefingRepository.observeAttentionItems() 订阅使用。
     * 任意一条跨角色关系（jealousy/tension）的变化都会触发这里重新查询。
     */
    @Query("SELECT * FROM relationship_states WHERE isInterCharacter = 1")
    fun observeAllInterCharacter(): Flow<List<RelationshipEntity>>

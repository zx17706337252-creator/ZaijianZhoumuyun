...
class PregnancyAnswerConsistencyChecker(
        ...
        return try {
            val response = provider.chatSyncWithRetry(
                ...
                config = LLMConfig(
                    ...
                    // buildRequestBody 中通过 config.model.ifEmpty { defaultModel } 兜底
                    // （即用户在设置里选的 provider 当前配置的模型，如火山方舟 + deepseek-v4-flash）。
                    // 注意 defaultModel 兜底行为取决于各 Provider 实现，并非 LLMProvider
                    // 接口层的统一契约；这样切换 provider/model 时本组件不需要任何改动。
                    model = "",
                    maxTokens = 32,
                    temperature = 0.2f,
                    stream = false,
    ...
    private fun buildUserPrompt(
        ...
        appendLine()
        appendLine("【上一次回答】")
        appendLine(previousAnswer.trim())
        appendLine()
        appendLine("【本次回答】")
        appendLine(newAnswer.trim())
        appendLine()

...
class SpecialtyEvolutionEngine(
    ...
    /** 剥离 soulNote 融合输出中 LLM 可能附带的前后缀噪音（Markdown 代码块、客套话等） */
    private fun stripSoulNoteNoise(raw: String): String {
        var text = raw.trim()
        // 去掉 Markdown 代码块包裹
        val fencePattern = Regex("^```[\\w]*\\n?|\\n?```$", RegexOption.MULTILINE)
        text = text.replace(fencePattern, "").trim()
        // 去掉常见 LLM 客套前缀
        val noisePrefixes = listOf("好的，", "以下是", "根据", "好的，以下是", "整合后的", "这是")
        for (prefix in noisePrefixes) {
            if (text.startsWith(prefix)) {
                text = text.removePrefix(prefix).trimStart()
                // 如果前缀后紧跟冒号或换行，也去掉
                if (text.startsWith("：") || text.startsWith(":")) {
                    text = text.removePrefix("：").removePrefix(":").trimStart()
                }
                break
            }
        }
    ...
    suspend fun matchCandidateObservation(
        ...
        try {
            ...
            try {
                ...
                // 依然失败（没有数字/数字越界）则走下面统一的"全新特征"兜底。
                extractFirstIndex(response)?.let { idx ->
                    if (idx in existingTraits.indices) existingTraits[idx] else null
                }
            }
        } catch (e: CancellationException) {
    ...
    /** matchCandidateObservation 的 JSON 解析失败兜底：从原始文本中提取第一个出现的整数 */
    private fun extractFirstIndex(raw: String): Int? =
    ...
    suspend fun integrateIntoSoulNote(
        ...
        try {
            val raw = provider.chatSyncWithRetry(
                messages = listOf(LLMMessage("user", userPrompt)),
                systemPrompt = systemPrompt,
                config = LLMConfig(
                    model = "",
            ...
            // 剥离 LLM 可能附带的前后缀噪音（Markdown 代码块、客套话等）
            ...
            if (newContent.length <= SpecialtyEvolutionConfig.SOUL_NOTE_MAX_CHARS) {
                newContent
            } else {
                // 超出上限时截断旧内容，保留新 trait 完整
                val maxOldLen = SpecialtyEvolutionConfig.SOUL_NOTE_MAX_CHARS - promotedTraitDescription.length - 2
                if (maxOldLen > 0) {
                    currentSoulNote.take(maxOldLen) + "\n\n" + promotedTraitDescription.trim()
                } else {
                    promotedTraitDescription.take(SpecialtyEvolutionConfig.SOUL_NOTE_MAX_CHARS)
                }
            }
        }
    }
    ...
    // ─────────────────────────────────────────────────────────
    //  ⑥.5 晋升标签浓缩（角色详情页"擅长领域"标签墙数据源）
    // ─────────────────────────────────────────────────────────

    /**
     * 将已晋升的完整特征描述（traitSummary，通常是一整句话，如"擅长用比喻
     * 营造画面感"）浓缩成一个适合在标签墙展示的短标签（2-4字，如"擅长比喻"）。
     *
     * 调用时机：仅在 IdentityPromotionEvaluator.executePromotion 用户确认
     * 晋升成功之后调用一次（低频，晋升本身就不频繁，成本可接受）。
     *
     * 与 matchCandidateObservation 同款容错策略：解析失败时不抛异常、
     * 不阻断晋升主流程——晋升写入 soulNote 是主线，标签墙展示是辅助功能，
     * 辅助功能失败不应该影响主线。失败时用 domain 兜底
     * （退化为"直接用专长方向名"的效果，好过标签墙完全没有这次晋升的痕迹）。
     */
    suspend fun distillSkillTag(
        domain: String,
        traitSummary: String,
    ): String = withContext(Dispatchers.IO) {
        val systemPrompt = """
            请把一段"角色擅长特征"的完整描述，浓缩成一个适合展示在标签墙上的
            极简标签，要求：
            1. 严格控制在2-4个汉字（不要标点、不要动词以外的修饰语）。
            2. 优先提炼具体的"能力/风格"关键词本身，而不是重复"擅长""很会"
               这类修饰词——例如"擅长用比喻营造画面感"应提炼为"比喻"或"画面感"，
               而不是"擅长比喻"。
            3. 只输出这一个词，不要任何解释、标点或引号。
        ...
        try {
            ...
            // 防御：模型偶尔仍会给出过长的短语，硬截断到4字上限，
            // 空字符串则视为提炼失败，走下面的 catch 分支同款兜底

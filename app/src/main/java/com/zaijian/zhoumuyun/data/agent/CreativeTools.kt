 ...
 *
 * 实现：调用 LLM（通过 [providerFn] 取当前激活 Provider），
 * 使用代码生成专用 System Prompt（简洁、仅输出代码块）。
 *
 * 结果回注给主 LLM，由角色以第一人称汇报结果。
 */
class CodeGenTool(
    private val providerFn: () -> LLMProvider? = AgentTool.defaultProviderFn(),
) : AgentTool {

    override val name      = "code_gen"
    override val description = "按语言和需求描述生成代码片段"
...
class CodeReviewTool(
    ...
    override val description = "对给定代码做结构化审查，给出问题列表、改进建议和评分"

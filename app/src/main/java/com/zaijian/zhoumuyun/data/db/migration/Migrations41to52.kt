...
//
// W1-003 修复：本迁移原先直接对重建后的 evolution_plans_new 做全量
// INSERT INTO ... SELECT，未做去重就建 UNIQUE INDEX。若旧表中存在重复
// (specialtyId, version) 行（历史并发写入残留），CREATE UNIQUE INDEX 会
// 抛 SQLite 约束错误，导致用户从 v43 升级到 v44 时 App 直接崩溃、无法
// 启动。参照 MIGRATION_39_40 对 judge_profiles 做同类升级时的正确做法：
// 在建唯一索引前，先按 (specialtyId, version) 分组只保留 createdAt 最早
// 的一条，删除其余重复行。pregnancy_answers 一并做同样处理。
internal val MIGRATION_43_44 = object : Migration(43, 44) {
    override fun migrate(db: SupportSQLiteDatabase) {
        // ── evolution_plans：先去重，再重建以加入唯一索引 ─────────

        // 去重：每组 (specialtyId, version) 只保留 createdAt 最早的一条。
        // 与 MIGRATION_39_40 同款兼容写法（GROUP BY + 子查询，不依赖
        // minSdk=26 系统 SQLite 版本不保证支持的窗口函数）。
        db.execSQL("""
            DELETE FROM evolution_plans
            WHERE id NOT IN (
                SELECT keep_id FROM (
                    SELECT id AS keep_id
                    FROM evolution_plans AS ep
                    WHERE ep.id = (
                        SELECT id FROM evolution_plans AS inner_ep
                        WHERE inner_ep.specialtyId = ep.specialtyId
                          AND inner_ep.version = ep.version
                        ORDER BY inner_ep.createdAt ASC, inner_ep.id ASC
                        LIMIT 1
                    )
                )
            )
        ...
        // 同理，若存在重复 (motherCharacterId, questionType, slotIndex,
        // answeredAt) 组合，建唯一索引前也需要先去重，否则同样会崩溃。
        db.execSQL("""
            DELETE FROM pregnancy_answers
            WHERE id NOT IN (
                SELECT keep_id FROM (
                    SELECT id AS keep_id
                    FROM pregnancy_answers AS pa
                    WHERE pa.id = (
                        SELECT id FROM pregnancy_answers AS inner_pa
                        WHERE inner_pa.motherCharacterId = pa.motherCharacterId
                          AND inner_pa.questionType = pa.questionType
                          AND inner_pa.slotIndex = pa.slotIndex
                          AND inner_pa.answeredAt = pa.answeredAt
                        ORDER BY inner_pa.id ASC
                        LIMIT 1
                    )
                )
            )
...
//
// W1-011 核查结论：本迁移第 4 步（UPDATE ... WHERE 1）是
// MIGRATION_40_41 的 content 反查错配问题的最终兜底修复，详见
// Migrations31to40.kt 中 MIGRATION_40_41 定义处的完整核查说明。
...
// P2-5 修复：practice_records 新增 (specialtyId, digestStatus, createdAt)
// 三列复合索引，覆盖 getOldestRawRecords 的 WHERE+ORDER BY 查询模式。
// 纯新增索引，不改表结构、不改现有数据，无需重建表。
internal val MIGRATION_50_51 = object : Migration(50, 51) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("CREATE INDEX IF NOT EXISTS `index_practice_records_specialtyId_digestStatus_createdAt` ON `practice_records` (`specialtyId`, `digestStatus`, `createdAt`)")
    }
}

// 擅长领域标签墙接通真实数据：新增 promoted_skill_tags 表。
// 角色详情页"能力"Tab 此前读取的是硬编码占位符，与专长进化系统的
// 晋升流程完全没有接通。本表由 IdentityPromotionEvaluator.executePromotion
// 在用户确认晋升时写入一条记录（SpecialtyEvolutionEngine.distillSkillTag()
// 浓缩出的2-4字短标签），CharacterDetailAbility.getSkillTags() 据此查询
// 真实数据。纯新增表，不涉及任何现有表结构或数据变更。
internal val MIGRATION_51_52 = object : Migration(51, 52) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            """
            CREATE TABLE IF NOT EXISTS `promoted_skill_tags` (
                `id` TEXT NOT NULL,
                `characterId` INTEGER NOT NULL,
                `specialtyId` TEXT NOT NULL,
                `tag` TEXT NOT NULL,
                `sourceTraitSummary` TEXT NOT NULL,
                `createdAt` INTEGER NOT NULL,
                PRIMARY KEY(`id`)
            )
            """.trimIndent()
        ...
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_promoted_skill_tags_characterId` ON `promoted_skill_tags` (`characterId`)"
        )
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_promoted_skill_tags_specialtyId` ON `promoted_skill_tags` (`specialtyId`)"
        )
    }
}

...
class TaskRepository(
     ...
     * 7.7 修复：ProjectViewModel.openProject() 今日规划实时订阅原直连 taskDao，
     * 现改经 TaskRepository 包装，统一数据访问路径。
     ...
     * 7.7 修复：ProjectViewModel.loadRecentGrowthSummary() 原直连 taskDao，
     * 现改经 TaskRepository 包装。
     * P2-B 成长记录历史摘要计算使用。
     */
    suspend fun getByProjectAndSourceAfter(
        ...
        taskDao.getByProjectAndSourceAfter(projectId = projectId, source = source, after = after)
    ...
    /**
     * 标记任务完成，产生 TASK_COMPLETED 事件。
     *
     * 批次6 6-2修复：原注释"产生 TASK_COMPLETED 事件（触发 MemoryCandidate 生成）"
     * 暗示事件本身驱动 MemoryCandidate 生成，但实际 MemoryEngine 没有订阅
     * world_events 的任何逻辑，MemoryCandidate 生成靠调用方（WorkbenchTaskTools）
     * 在调用 completeTask() 之后**再单独调用** memoryEngine()?.onTaskCompleted()
     * 完成。当前只有 WorkbenchTaskTools 一个调用方所以不会出问题，但若未来有
     * 新调用方信任旧注释而不复刻这次额外调用，会导致记忆链路静默断裂。
     * 改为准确描述：事件写入不等于记忆生成，后者由调用方显式调用 MemoryEngine。
     */
     ...
     * 7.7 修复：ProjectViewModel.toggleGrowthTask() 原直连 taskDao，现改经
     * TaskRepository 包装。不写 WorldEvent——成长任务勾选是低重要度、高频次
     * 的 UI 交互，与 completeTask() 走完整事务+事件链路的"正式完成任务"
     * 语义不同，避免记忆链路被大量勾选操作污染。
     * P2-A/B TodayGrowthSection 勾选框交互使用。
     */
    suspend fun toggleGrowthTaskDone(id: String, now: Long = System.currentTimeMillis()) {
        taskDao.toggleGrowthTaskDone(id = id, now = now)
    }

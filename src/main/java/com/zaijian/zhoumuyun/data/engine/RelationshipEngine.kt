...
class RelationshipEngine(
    ...
    suspend fun onConversationEnd(characterId: Int, messageCount: Int, sourceEventId: String) {
        val delta = RelationshipDelta(
            affection = if (messageCount >= 4) 1 else 0,
            curiosity = 1,
        )
        if (delta.affection > 0 || delta.curiosity > 0) {
            applyDelta("user", characterId.toString(), delta, sourceEventId)
        }
    }
            ...
            speakerIds.forEach { speakerId ->
                if (watcherId != speakerId) {
                    ...
                    relationshipDao.updateInterCharacterDynamics(
                        fromId    = watcherId.toString(),
                        toId      = speakerId.toString(),
    ...
    suspend fun buildInterCharacterSnapshot(
        ...
        val lines = memberIds.mapNotNull { otherId ->
            // Bug 4 修复：用 getBetween 查 isInterCharacter=1 的角色间记录（含双向），
            // 优先取 forCharacterId→otherId 方向；旧 get() 无 isInterCharacter 过滤，
            // 会随机命中用户-角色记录，导致关系数据错乱。
            // 若记录尚未初始化（空列表），自动创建初始记录（stage=STRANGER，各维度为0），
            // 保证所有在场角色都出现在 snapshot 中，不因数据缺失而被跳过。
            val pairs = relationshipDao.getBetween(forCharacterId.toString(), otherId.toString())
            val rel = pairs.firstOrNull { it.fromId == forCharacterId.toString() }
                ?: pairs.firstOrNull()
                ?: getOrCreate(forCharacterId.toString(), otherId.toString(), interCharacter = true)

...
object IdentityPromotionEvaluator {
     ...
     */
    fun boostStability(profileId: String, trait: String) {
    ...
    suspend fun executePromotion(
        ...
        val profile = db.specialtyProfileDao().getById(profileId) ?: return
        val identity = db.characterIdentityDao().getById(profile.characterId)
        ...
        // LLM 调用（integrateIntoSoulNote）在事务外完成，只有结果写入在事务内。
        db.withTransaction {
            // 写入 soulNote：P0-2 修复，改用事务化单列 upsert，消除并发 REPLACE 竞态。
            // 自动获得 P5 Step3 的备份+撤销能力（updateSoulNote 内部已同步写入 soulNoteBackup）。
            db.characterIdentityDao().upsertSoulNote(profile.characterId, integratedSoulNote)

...
//
// W1-011 核查结论（已被后续迁移修复，无需改动）：
// 下面的回填用 memories.content 反查 memories_fts.rowid（content 非唯一列，
// LIMIT 1 在有重复 content 的情况下会取到错误的匹配行，导致部分记忆的
// ftsRowId 被错误赋值）。该问题已在 MIGRATION_44_45（本文件所在的
// Migrations41to52.kt）第 188-192 行被彻底兜底修复：
//   UPDATE `memories` SET `ftsRowId` = `rowid` WHERE 1
// 这条语句用 `WHERE 1`（恒真条件）无条件覆盖了 memories 表的每一行，
// 而不仅仅是 `ftsRowId = 0` 的行——也就是说，无论 MIGRATION_40_41 当初
